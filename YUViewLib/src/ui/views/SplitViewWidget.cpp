...
// From the MS docs: "The standard DPI settings are 100% (96 DPI), 125% (120 DPI), and 150% (144
// DPI). The user can also apply a custom setting. Starting in Windows 7, DPI is a per-user
// setting." For 96 a divisor of 24 will results in +-4 pixels and 150% will result in +-6 pixels.
...
void splitViewWidget::setHDRRenderingMode(HDRRenderingMode mode, bool callUpdate)
{
  ...
  if (mode == HDRRenderingMode::Enabled)
  {
    ...
    // The MacEDR widget initializes its Metal renderer lazily (on first show/paint).
    // We always create it and show it when HDR is enabled on macOS.
    ...
    // Check EDR mode preference from settings
    ...
    // Also create OpenGL HDR10Widget as fallback (always needed for SDR path)
    if (!hdr10Widget)
    {
...
#ifdef Q_OS_MAC
    // On macOS: if MacEDR is preferred and active, hide the OpenGL widget
    // (MacEDR handles EDR; OpenGL widget is just a fallback)
    ...
    {
      // Mac EDR not available, show OpenGL widget instead
      hdr10Widget->show();
      if (hdr10WidgetMacEDR)
...
void splitViewWidget::paintEvent(QPaintEvent *)
{
  ...
  else // (!splitting)
  {
    ...
    if (item[0])
    {
      ...
      // HDR rendering: Use HDR10Widget or MacEDR widget if enabled
      bool useMacEDRWidget = false;

#ifdef Q_OS_MAC
      // macOS: Check if user wants Metal EDR mode
      QSettings edrSettings;
      bool edrModePref = edrSettings.value("View/EDRMode", true).toBool();

      // macOS: use MacEDR widget (Metal-based) for EDR when EDR mode is enabled
      if (hdrRenderingMode == HDRRenderingMode::Enabled && edrModePref && hdr10WidgetMacEDR && !waitingForCaching)
      {
        // Position MacEDR widget to cover the drawing area
        hdr10WidgetMacEDR->setGeometry(0, 0, width(), height());
        ...
        useMacEDRWidget = true;
      }
#endif

      if (!useMacEDRWidget && hdrRenderingMode == HDRRenderingMode::Enabled && hdr10Widget && !waitingForCaching)
      {
      ...
      // (HDR10Widget or MacEDR widget already rendered the frame)
      bool hdrActive = (hdrRenderingMode == HDRRenderingMode::Enabled);
#ifdef Q_OS_MAC
      hdrActive = hdrActive && (useMacEDRWidget || hdr10Widget);
#endif
...
void splitViewWidget::updatePixelPositions()
{
  ...
  if (anyItemsSelected && this->drawZoomBox && geometry().contains(this->zoomBoxMousePosition))
  {
    ...
    const auto itemCenterMousePos = (mouseInLeftOrRightView) ? centerPoints[1] + this->moveOffset
                                                             : centerPoints[0] + this->moveOffset;
...
void splitViewWidget::setZoomBoxPixelUnderCursor(QPoint posA,
                                                 QPoint posB,
...
void splitViewWidget::mouseMoveEvent(QMouseEvent *mouse_event)
{
  ...
  if (this->drawZoomBox)
  {
    // If the mouse position changed, save the current point of the mouse and update the view (this
    // will update the zoom box)
...
void splitViewWidget::mousePressEvent(QMouseEvent *mouse_event)
{
  if (mouse_event->source() == Qt::MouseEventSynthesizedBySystem &&
    ...
    // The mouse event was generated by the system from a touch event which is already handled by
    // the touch pinch handler.
    return;
...
bool splitViewWidget::updateMouseCursor(const QPoint &mousePos)
{
  ...
  if (this->viewAction == ViewAction::NONE)
  {
    ...
    if (isSplitting())
    {
      ...
      else if ((mousePos.x() < splitPosPix && item[0] && item[0]->isLoading()) ||
               (mousePos.x() > splitPosPix && item[1] && item[1]->isLoading()))
        // Mouse is not over the splitter line but the item that the mouse is currently over is
        // loading.
        setCursor(Qt::BusyCursor);
      else if (mouseMode == MOUSE_LEFT_MOVE)
...
void splitViewWidget::gridSetCustom(bool)
{
  bool ok;
...
void splitViewWidget::zoomToFitInternal()
{
  ...
  // We have a fractional zoom factor but we can only set multiples of
  // SPLITVIEWWIDGET_ZOOM_STEP_FACTOR. Find the next SPLITVIEWWIDGET_ZOOM_STEP_FACTOR multitude that
  // fits.
...
void splitViewWidget::currentSelectedItemsChanged(playlistItem *item1, playlistItem *item2)
{
  ...
  if (savePositionAndZoomPerItem)
  {
    ...
    if (item1)
    {
      ...
      item1->getZoomAndPosition(this->getOtherWidget()->moveOffset,
    ...
    else if (item2)
    {
      ...
      item2->getZoomAndPosition(this->getOtherWidget()->moveOffset,
                                this->getOtherWidget()->zoomFactor,
                                getOtherViewValuesFromOtherSlot);
    }
...
void splitViewWidget::update(bool newFrame, bool itemRedraw)
{
  ...
  if (newFrame || itemRedraw)
  {
    ...
    if ((itemLoading[0] || itemLoading[1]) && playing)
      ...
      return;
    // We only need to redraw the items if a new frame is now loading and the "Loading..." message
    // was not drawn yet.
