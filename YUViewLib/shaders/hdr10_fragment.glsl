...
//   6. sRGB OETF (linear → sRGB curve + gamut) for SDR display
//   7. Output RGBA8 (0-1 range, sRGB encoded)
//
// For EDR/HDR output on macOS, use the Metal-based HDR10WidgetMacEDR instead.
...
vec3 hlgEotf(vec3 hlg) {
    ...
    // Scale so peak = 4 × diffuse white (matching Metal EDR path)
    return linear * 4.0;
}
...
// Converts linear light back to sRGB curve + gamut for SDR display
...
void main()
{
    ...
    // ========== sRGB OETF (for SDR output) ==========
    // Encode back to sRGB curve + gamut for standard SDR display
